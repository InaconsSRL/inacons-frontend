...
const FormularioPresupuesto: React.FC<FormularioPresupuestoProps> = ({
    ...
    const [formData, setFormData] = useState<FormState>({
        ...
        total_presupuesto: '0',
        numeracion_presupuesto: cantidadPresupuestos + 1, // Esto ahora usará la cantidad correcta del proyecto actual
    });
    ...
    useEffect(() => {
        if (presupuesto) {
            setFormData({
                ...
                numeracion_presupuesto: presupuesto.numeracion_presupuesto || cantidadPresupuestos + 1
            });
        }
    }, [presupuesto]);
        ...
        <form onSubmit={handleSubmit} className="bg-gray-900/70 p-4 sm:p-6 md:p-8 rounded-lg max-w-4xl mx-auto">
            <div className="space-y-4 sm:space-y-6">
                <div className="grid grid-cols-1 gap-4 sm:gap-6">
                    ...
                    <InputField
                            label="Orden"
                            name="numeracion_presupuesto"
                            type="number"
                            step="1"
                            min={1}
                            max={cantidadPresupuestos + 1}
                            value={formData.numeracion_presupuesto?.toString() || ''}
                            onChange={handleChange}
                        />
