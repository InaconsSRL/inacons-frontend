...
interface LeftPanelProps {
    ...
    setSelectedPresupuesto: (presupuestoId: string) => void;
    selectedPresupuesto: string | null;
}
...
const LeftPanel: React.FC<LeftPanelProps> = ({
    ...
    const renderNode = (node: CostoUnitarioNode) => {
        ...
        const textColor = node.isPresupuestoRoot ? 'text-blue-600 font-medium' : getColorClass(node.level, node.estado_expandido);
                ...
                <div
                    ...
                    onClick={() => {
                        if (node.isPresupuestoRoot) {
                            setSelectedPresupuesto(node.id_presupuesto);
                        }
                ...
                >
                    <div className="flex items-center text-xs">
                        ...
                        <span className={`text-[10px] ${textColor}`}>
                            {node.isPresupuestoRoot ? node.descripcion_costo : `${node.numeracion_costo} - ${node.descripcion_costo}`}
                        </span>
                    </div>
                </div>
    ...
    return (
        <div className="w-1/3 border-r border-gray-200 overflow-y-auto overflow-x-auto bg-white h-[calc(100vh-155px)]">
            <div className="px-4 py-3 border-b border-gray-200 bg-gray-50/50 min-w-[300px]">
                <h2 className="text-sm font-medium text-gray-700">Presupuestos</h2>
            </div>
