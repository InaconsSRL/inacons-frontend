...
const NewRecepcionTransferencia: React.FC<ModalProps> = ({ onClose }) => {
  ...
  const loadRecursos = async () => {
   ...
   try {
    ...
    const response = await dispatch(
     fetchTransferenciaRecursosById(selectedTransferencia)
    );
    ...
    const recursosConCantidad = response.payload.map(
     (recurso: Recurso) => ({
      ...recurso,
    ...
    if (obraDestinoId) {
     const bodegasPromises = response.payload.map(
      async (recurso: TransferenciaRecurso) => {
       try {
        const bodegas =
         await consultaRecursosPorBodegaByObraIdAndRecursoIdService({
        ...
        console.error(
         `Error al obtener bodegas para recurso ${recurso.recurso_id.id}:`,
         error
        );
     ...
     const bodegasData = bodegasResults.reduce(
      (acc, curr) => ({
       ...acc,
       ...curr,
      }),
 ...
 useEffect(() => {
  const loadObrasInfo = async () => {
   if (selectedTransferencia) {
    ...
    if (transferencia) {
     const response = await dispatch(
      fetchObraOrigenYDestino(transferencia.transferencia_id.id)
     );
 ...
 const handleCantidadChange = (
  ...
  if (recurso) {
   setSelectedRecursos((prev) => {
    ...
    const totalDistribuido = Object.values(nuevaDistribucion).reduce(
     (sum, val) => sum + val,
     0
    );
 ...
 const handleSave = async () => {
  try {
   ...
   const detalleTransferencia = await dispatch(
             ...
             <td className="px-2 py-2">
              <div className="space-y-1">
               {bodegasDelRecurso.map((bodega) => (
                ...
                >
                 <span
                  className="text-xs text-gray-600 w-24 truncate"
                  title={bodega.nombre}
                 >
                  {bodega.nombre}:
                 </span>
                 <input
                  ...
                  className="w-20 px-1 py-0.5 text-xs border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                 />
               ...
               <div className="text-xs text-indigo-600">
                Total: {totalDistribuido} / {recurso.cantidad}
               </div>
              </div>
             </td>
            </tr>
