...
import KanbanCardAprobacionSupervisor from './KanbanCardAprobacionSupervisor';

interface KanbanColumnProps {
  column: Column;
}

const KanbanColumnAprobacionSupervisor: React.FC<KanbanColumnProps> = ({ column }) => {
    return (
      <div className="flex-shrink-0 w-auto bg-white/10 rounded-lg shadow-lg scale-90 lg:scale-100 max-h-[80vh] ">
        ...
        <div className="p-4 space-y-4 max-h-[calc(80vh-200px)] overflow-y-auto">
          {column.tasks.map((task) => (
            <KanbanCardAprobacionSupervisor key={task.id} task={task} />
          ))}
        </div>
      </div>
    );
...
export default KanbanColumnAprobacionSupervisor;