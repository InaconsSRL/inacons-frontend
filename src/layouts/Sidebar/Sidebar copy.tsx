...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen }) => {
  ...
  return (
    <div
      className={`bg-black/50 backdrop-blur-lg transition-all duration-500 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-16'
        }`}
    >
      <nav className="flex flex-col items-start p-4 h-full overflow-y-auto">
        ...
        <div className="flex flex-col w-full ">
          <div
            className="flex items-center justify-between space-x-4 cursor-pointer mb-6"
          ...
          <div
            className={`
              ${isCollaborationOpen ? "rounded-[10px] shadow-neumorph max-h-screen" : "max-h-0"}
              pt-3 -m-2 pl-2 overflow-hidden transition-all duration-300 }`}
          >
            ...
            <NavLink to="/dashboard/requerimiento" className={({ isActive }) => 
                `flex items-center space-x-4 mb-4 ${isActive ? activeStyle : ''}`
              }>
              <FiPackage className="w-6 h-6 text-white" />
              {isSidebarOpen && <span className="text-white">Requerimiento</span>}
            </NavLink>
